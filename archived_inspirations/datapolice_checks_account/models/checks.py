    ...
    def check_product_account(self):
        for rec in self:
            for company in rec.env['res.company'].search([]):
                if not rec.with_context(force_company=company.id).property_account_income_id:
                    raise Exception("Property Account Incoming missing for {} {} {}".format(company.name, rec.default_code or '', rec.name or ''))
